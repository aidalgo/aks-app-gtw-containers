...
INGRESS_FRONTEND_NAME=$(terraform -chdir="$TF_DIR" output -raw ingress_frontend_name)
...
echo "    Ing. frontend name : $INGRESS_FRONTEND_NAME"
...
sed -e "s|__ALB_ID__|$ALB_ID|g" \
    -e "s|__INGRESS_FRONTEND_NAME__|$INGRESS_FRONTEND_NAME|g" \
    "$SCRIPT_DIR/k8s/test-app-ingress-byo.yaml" > "$ING_FILE"
